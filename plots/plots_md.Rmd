...
parse <- function(x){
  lapply(strsplit(x, " "), as.set)
}

for(p in problems){
  df_temp <- df %>%
    filter(problem == p) %>% ## dodac optimal !!!
    mutate(quality = (optimal_cost-final_cost)/optimal_cost) %>%
    mutate(similarity = set_similarity(parse(final_path), parse(optimal_path))) %>%
    select(c("problem", "optimizer", "quality"))
  ...
  ggsave(sprintf("plots_res/final/all_mean_min/ex5_%s.pdf", p), plot_temp, width = 8, height = 4)
