...
ggsave("plots_res/final/opt_comp_quality.pdf", plot_comb, width=12, height=5, device=cairo_pdf)
...
ggsave("plots_res/final/opt_comp_time.pdf", plot_comb, width=12, height=5, device=cairo_pdf)
```
...
ggsave("plots_res/final/opt_comp_qual_time.pdf", plot_steps, width=7, height=5, device=cairo_pdf)
...
ggsave("plots_res/final/opt_comp_steps.pdf", plot_comb, width=7, height=5, device=cairo_pdf)
...
ggsave("plots_res/final/opt_comp_seen.pdf", plot_comb, width=12, height=8, device=cairo_pdf)
...
ggsave("plots_res/final/start_vs_end.pdf", plot_steps, width=10, height=8, device=cairo_pdf)
...
for(p in problems){
  ...
  ggsave(sprintf("plots_res/final/all_mean_min/mean_min_score_%s.pdf", p), plot_temp, width = 8, height = 4, device=cairo_pdf)
}
...
as.pairs <- function(x){
  y <- c(x[-1], x[1])
  return(sets::as.set(Map(sets::set, x, y)))
}

similarity2 <- function(x1, x2){
  s1 <- sets::as.pairs(parse2(x1))
  s2 <- sets::as.pairs(parse2(x2))
  
  return(length(sets::set_intersection(s1, s2)) / length(s1))
}
...
for(p in problems){
  df_temp <- df %>%
    ...
    mutate(quality = normalize((optimal_cost-final_cost)/optimal_cost)) %>%
    top_n(n=100) %>%
    mutate(similarity = pmap_dbl(list(final_path, optimal_path), similarity2)) %>%
    # mutate(similarity = simi(parse(final_path), parse(optimal_path))) %>%
    select(c("problem", "optimizer", "quality", "similarity"))
